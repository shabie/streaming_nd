...
async def consume():
    ...
    data = {
        "name": consumer_name,
        "format": "avro",  # i.e. we want to consume in Avro format
    }

    resp = requests.post(
        f"{REST_PROXY_URL}/consumers/{CONSUMER_GROUP}",  # TODO: Add consumer group to the URL
    ...
    resp = requests.post(
        f"{resp_data['base_uri']}/subscription",  # TODO: Add "subscription" to the URI
        data=json.dumps(data),
        headers=headers,
    )
    try:
    ...
    while True:
        ...
        # TODO: Add "records" to the URI and also set "timeout" query parameter (measured in milliseconds) to 10 seconds
        resp = requests.get(f"{resp_data['base_uri']}/records?timeout=10000",
                            headers=headers,
                            )
        try:
            resp.raise_for_status()
